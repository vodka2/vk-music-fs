...
public:
    ...
    MOCK_CONST_METHOD1(prepend, void (ByteVect vect)); //NOLINT
...
TEST_F(Mp3ParserT, AddID3){ //NOLINT
    ...
    init(buf);
    ByteVect prepVect;
    EXPECT_CALL(*inj.create<std::shared_ptr<BlockingBufferM>>(), prepend(testing::_)).WillOnce(
            testing::Invoke([&prepVect] (auto vect) {
                prepVect = vect;
            })
    );
    ...
    std::copy(fileMp3NoID3, fileMp3NoID3 + fileMp3NoID3Len, std::back_inserter(vect));
...
TEST_F(Mp3ParserT, DontAddID3){ //NOLINT
    ...
    init(buf);
    EXPECT_CALL(*inj.create<std::shared_ptr<BlockingBufferM>>(), prepend(testing::_)).Times(0);
    inj.create<std::shared_ptr<Mp3Parser>>()->parse<BlockingBufferM>(inj.create<std::shared_ptr<BlockingBufferM>>());
    std::vector<uint8_t> vect;
    ...
    TagLib::ByteVectorStream strm(bvect);
    TagLib::MPEG::File f(&strm, TagLib::ID3v2::FrameFactory::instance());
    EXPECT_EQ(f.ID3v2Tag(false)->title(), std::string("World"));
    EXPECT_EQ(f.ID3v2Tag(false)->artist(), std::string("Hello"));
}